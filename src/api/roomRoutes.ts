  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const { rows } = await client.query(
        `
      INSERT INTO room_info (room_desc, is_private, auto_speaker, creator_id, chat_enabled, hand_raise_enabled)
      VALUES ($1, $2, $3, $4, $5, $6)
      RETURNING room_id;
        `,
        [
    ...
    await Promise.all(
      categories.map(async categoryId => {
        await client.query(
          `
            INSERT INTO room_info_category (room_id, category_id)
            VALUES ($1, $2);
          `,
          [roomId, categoryId]
        );
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const { rows: room_info } = await pool.query(
      `
      SELECT *
      FROM room_info
      WHERE room_id = $1;
    ...
    try {
      ...
      const { rows: room_info } = await client.query(
        `
          SELECT *
          FROM room_info
          WHERE room_id = $1;
      ...
      await client.query(
        ...
        [
          roomId,
          userId,
          parseCamel(room_info[0])?.autoSpeaker ||
            parseCamel(room_info[0])?.creatorId == userId,
          parseCamel(room_info[0])?.creatorId === userId,
          false,
          true,
        ]
      ...
      const { rows: categories } = await client.query(
        `
        SELECT c.name
        FROM room_info_category AS ric
        JOIN categories AS c ON ric.category_id = c.category_id
        WHERE ric.room_id = $1
        `,
        [roomId]
      );
      // const { rows: categories } = await client.query(
      //   `
      //     SELECT categories FROM room_info_category 
      //     WHERE room_id = $1
      //   `,
      //   [roomId]
      // );
...
router.get(
  "/rooms/live",
  async (req: Request, res: Response, next: NextFunction) => {
    logger.info("Fetching live rooms");
    try {
      const { rows: rooms } = await pool.query(
        `
        SELECT 
            room_info.*,
            (SELECT user_name FROM user_data WHERE user_id = room_info.creator_id) AS creator,
            (
                SELECT json_agg(json_build_object('user_name', ud.user_name, 'avatar_url', ud.avatar_url))
                FROM user_data ud
                WHERE ud.current_room_id = room_info.room_id
            ) AS participants,
            ARRAY(
                SELECT categories.name 
                FROM room_info_category 
                JOIN categories ON room_info_category.category_id = categories.category_id
                WHERE room_info_category.room_id = room_info.room_id
            ) AS categories
        FROM 
            room_info
        WHERE 
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      await client.query(
        `
          DELETE FROM room_info_category
          WHERE room_id = $1
        `,
        [roomId]
      );
      ...
      return res.status(200).json({ msg: "room_info destroyed" });
    } catch (error) {
      await client.query("ROLLBACK");
      next(error);
    } finally {
...
router.post(`/soft-delete`, async (req: Request, res: Response) => {
  ...
  res.status(200).json({ msg: "room_info soft ended" });
});
